...
class changeSuppQuotationStatus extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'suppQuotationStatus:change';
     ...
     * @var string
     */
    protected $description = 'This is a schedule for change supplier status after 5 minute  go get quotation , based on specific quote.';
        ...
        if(count($requestedQuotes)>0){
            foreach($requestedQuotes as $reqquote){
                if($reqquote->supplierid !=''){
                    // Get quotation  of that quote which is in Inprogress
                    ...
                    if(count($IprogressResps)>0){
                       foreach ($inProgressResps as $resp) {
                            ...
                            if ($resp->created_at->diffInHours(now()) > 3) {
                                $resp->status = 'Closed';
                                $resp->save();
